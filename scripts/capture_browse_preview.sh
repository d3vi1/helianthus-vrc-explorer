...
POSTER_PERCENT=45
FONT_SIZE=16
CAPTURE_CMD="PYTHONPATH=src \"${PYTHON_BIN}\" -m helianthus_vrc_explorer browse --file fixtures/demo_browse.json --allow-write"
...
mkdir -p "$OUTPUT_DIR"
TXT_PATH="$OUTPUT_DIR/browse.txt"
PNG_PATH="$OUTPUT_DIR/browse.png"
GIF_PATH="$OUTPUT_DIR/browse.gif"
CAST_PATH="$OUTPUT_DIR/browse.cast"

echo "Recording asciicast to $CAST_PATH ..."
ASCIINEMA_CMD='
  set -euo pipefail
  stty cols "$COLS" rows "$ROWS" >/dev/null 2>&1 || true
  PYTHON_BIN="$PYTHON_BIN" expect -f scripts/demo_browse_capture.exp
'
