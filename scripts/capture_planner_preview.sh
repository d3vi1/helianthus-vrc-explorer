...
POSTER_PERCENT=45
FONT_SIZE=16
CAPTURE_CMD="PYTHONPATH=src \"${PYTHON_BIN}\" scripts/demo_planner_textual.py"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: $0 [--capture-seconds 25] [--speedup 1] [--output-seconds 25] [--cols 140] [--rows 42] [--fps 20] [--poster-percent 45] [--font-size 16] [--output-dir artifacts/readme] [--font-path /path.ttf] [--command \"...\"]" >&2
      exit 2
      ;;
  esac
done
...
mkdir -p "$OUTPUT_DIR"
TXT_PATH="$OUTPUT_DIR/planner.txt"
PNG_PATH="$OUTPUT_DIR/planner.png"
GIF_PATH="$OUTPUT_DIR/planner.gif"
CAST_PATH="$OUTPUT_DIR/planner.cast"

echo "Recording asciicast to $CAST_PATH ..."
ASCIINEMA_CMD='
  set -euo pipefail
  stty cols "$COLS" rows "$ROWS" >/dev/null 2>&1 || true
  PYTHON_BIN="$PYTHON_BIN" expect -f scripts/demo_planner_capture.exp
'

PYTHON_BIN="$PYTHON_BIN" COLS="$COLS" ROWS="$ROWS" TERM="xterm-256color" \
  asciinema rec \
    --overwrite \
    --headless \
